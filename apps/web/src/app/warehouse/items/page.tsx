...
/*
 * Postavke — pregled zaloge/artiklov. Iskanje gre prek pravega
 * /inventory/items, vrednost zaloge prek /warehouse-reports/valuation.
 */
export default function ItemsPage() {
  ...
  useEffect(() => { api.warehouseReports.valuation().then(setValuation).catch(() => setValuation(null)); }, []);
  useEffect(() => {
        ...
        {items === null ? <div className="p-6"><Spinner className="text-brand" /></div>
          ...
          : (
            <table className="w-full text-sm">
              ...
              <tbody>
                {items.map((it, i) => (
                  <tr key={it.id ?? i} className="border-t border-line">
