    ...
    <div className="grid min-h-screen grid-cols-1 bg-floor lg:grid-cols-[15.5rem_1fr]">
      ...
      <div className="flex min-w-0 flex-col pb-[6.5rem] lg:pb-0">
        ...
        <MobileTabBar items={[
          ...
          { label: 'Iskanje', icon: 'search', onClick: focusAdvisorSearch },
          { label: 'Nov nalog', icon: 'plus', href: '/advisor/work-orders', fab: true },
          { label: 'Obvestila', icon: 'bell', onClick: openAdvisorBell },
...
/** Skoči gor in fokusira ukazno vrstico (#advisor-command). */
function focusAdvisorSearch() {
  window.scrollTo({ top: 0, behavior: 'smooth' });
  (document.getElementById('advisor-command') as HTMLInputElement | null)?.focus();
}

/** Skoči gor in odpre zvonec obvestil (#advisor-bell). */
function openAdvisorBell() {
  window.scrollTo({ top: 0, behavior: 'smooth' });
