...
/**
 * Telefon (0030) lokalno razširja shared Customer — selitev polja v shared tip
 * ob naslednjem dotiku packages/shared (zaveden backlog), da ta sprint ne
 * zahteva rebuilda shared paketa.
 */
export type CustomerWithPhone = Customer & { phone: string | null };
...
function toDomain(r: CustomerRow): CustomerWithPhone {
  return {
...
export class CustomersRepository {
    ...
    const res = await tx.query<CustomerRow>(
      `INSERT INTO app.customers
         (id, tenant_id, code, name, type, country, address, post_code, city,
          ...
          discount_pct, minimax_partner_id, created_by, updated_by, phone)
      ...
      [
        ...
        c.discountPct, c.minimaxPartnerId, c.createdBy, c.phone,
      ],
    );
    return toDomain(res.rows[0]);
  }

  async findById(tx: TxClient, id: string): Promise<CustomerWithPhone | null> {
  ...
  ): Promise<CustomerWithPhone | null> {
    const res = await tx.query<CustomerRow>(
  ...
  async update(
    ...
    c: {
      ...
      discountPct?: string; minimaxPartnerId?: string | null; phone?: string | null;
    ...
    const res = await tx.query<CustomerRow>(
      ...
      [
        ...
        c.minimaxPartnerId ?? null, c.updatedBy, c.phone ?? null,
      ],
    );
    return res.rowCount > 0 ? toDomain(res.rows[0]) : null;
  }
  ...
  async list(tx: TxClient, limit: number, afterName?: string, afterId?: string, q?: string): Promise<CustomerWithPhone[]> {
